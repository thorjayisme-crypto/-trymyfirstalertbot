...
import re#把re工具从python里拿出来，用来做正则表达式匹配，在情绪分析预警里用于从gpt返回的文本里提取综合评分数字。

#拿到alchemyapi连接主网
rpc_list = [
    "https://eth-mainnet.g.alchemy.com/v2/lZy8XiMppx5tar2jviA0G",
    "https://clean-billowing-dew.quiknode.pro/f5708f23ae1ea578e626f7c8540b6fc36fbf31d5/",
    "https://mainnet.infura.io/v3/86c62c50d1ee4282860f04f3c35ee81a"
]#rpc_list是我们准备好的多个中间人的电话号码列表，里面有alchemy、quiknode、infura三个中间人，相当于我要找这些代理人中间人给我办事，后面的网址就是他们留下的电话号码，我要打这个电话号码才能找到他们帮我去以太坊主网查数据，防止某一个中间人挂了没法干活。

w3 = None
for rpc in rpc_list:
    try:
        w3_temp = Web3(Web3.HTTPProvider(rpc))#rpc相当于这个中间人的电话号码，就是那串网址，用web3.HTTPProvider给这个电话号码打电话找中间人去以太坊干活。
...
if w3 is None:
    ...
    exit()#w3表示这个过程电话接通了中间人随时待命，我们给这个状态取名叫w3，到时我要叫中间人去主网干活就发w3，随时调动他帮我去以太坊主网干活。如果所有中间人都联系不上，就停止程序。
...
try:
    ...
    print(f"✅ Chainlink ETH价格：${eth_price_chainlink}")
except Exception as e:
    print(f"⚠️ Chainlink获取失败：{e}，尝试用Pyth价格替代")
...
try:
    ...
    print(f"⚠️ Pyth获取失败：{e}，尝试用Chainlink价格替代")
...
if eth_price_chainlink is None and eth_price_pyth is None:
    ...
    print("⚠️ Chainlink不可用，用Pyth价格替代Chainlink")
    eth_price_chainlink = eth_price_pyth
elif eth_price_pyth is None:
    print("⚠️ Pyth不可用，用Chainlink价格替代Pyth")
    eth_price_pyth = eth_price_chainlink

#计算tvl_chainlink
steth_tvl_chainlink=eth_price_chainlink*total_supply_eth#把上面我们在链上通过alchemy拿到的steth的总锁仓量*预言机上拿到的eth/usd币对的实时最新价格的积就是steth_tvl_chainlink
print(steth_tvl_chainlink)#打印出来steth_tvl_chainlink的结果就是chainlink预言机计算出来的steth的美元价值总额，就是所谓tvl。
#计算tvl_pyth
steth_tvl_pyth=eth_price_pyth*total_supply_eth#用pyth预言机拿到的价格乘以我们在主网上拿到的steth锁仓数量的积就是steth_tvl_pyth
print(steth_tvl_pyth)#打印出来steth_tvl_pyth的结果就是pyth预言机计算出来的steth的美元价值总额，就是所谓tvl。
...
def fetch_lido_tweets():#定义这个去6551接口取推文过程的函数叫fetch_lido_tweets
    ...
    if response.status_code == 200:#如果拿到的数据里面的返回码是200，表示拿数据成功。
        return response.json().get("data", [])#把拿到的数据用json格式翻译给python读取，然后把拿到的这些数据打包成列表返回，如果拿到的数据里面没有内容，也返回成一个空列表备用。
    else:
        return []#如果拿到的数据里面的返回码不是200，表示拿数据失败，就返回一个空列表。

def analyze_lido_sentiment():#定义用gpt分析推文情绪的过程的函数。
    tweets = fetch_lido_tweets()#tweets代表上面fetch_lido_tweets()函数拿到的数据，是一个列表。
    if tweets == []:#如果这个函数拿到的数据没有内容，列表是空的。
        return "无法获取推文"#就返回这个反馈信息
    else:
        列表 = []#定义一个新的空列表，方便后面把推文内容放进来。
        for t in tweets:#用for/in循环把列表里面的每条推文数据定义为t，下面针对列表里的每个t轮流进行操作。
            line = f"- [{t['userScreenName']}] {t['text'][:500]}"#把每条推文t里面的数据处理成[用户名][推文前500个字]的形式。
            列表.append(line)#把处理的推文一条一条加入我们前面新建的这个空列表里面。
        tweet_texts = "\n".join(列表)#把列表按每一条自动换行的形式排列起来，存入tweet_texts变量，方便后面引用。

        response = client.chat.completions.create(#按照路径一步一步调用gpt的功能，创建一次对话请求。
            model="gpt-5.5",
            messages=[
                {
                    "role": "system",
...
4. 综合评分（1-10）"""
        ...
        result = response.choices[0].message.content#取gpt返回的第一条回答的完整内容。

        # 情绪预警检测
        print("====== X推文情绪预警检测 ======")
        for line in result.split("\n"):
            if "综合评分" in line:
                score_match = re.search(r'\d+', line)#用re工具在这一行文字里找数字，找到就返回匹配对象。
                if score_match:
                    score = int(score_match.group())#把匹配到的数字字符串转为整数，方便后面判断。
                    if score <= 3:
                        print(f"⚠️ 警告！市场情绪极度悲观！综合评分：{score}/10")
                    elif score <= 5:
                        print(f"⚠️ 注意！市场情绪偏负面，综合评分：{score}/10")
                    elif score >= 9:
                        print(f"⚠️ 注意！市场情绪过度亢奋，可能存在风险！综合评分：{score}/10")
                    else:
                        print(f"✅ 市场情绪正常，综合评分：{score}/10")
        print("==============================")

        return result#把gpt返回的回答内容return出来，方便后面引用。
# ====== 推特情绪分析函数定义结束 ======
...
if len(history1) < 2:
    print("⚠️ 历史数据不足，需要至少两条记录才能进行对比分析，跳过日环比计算")
    exit()
...
# ETH价格偏差对比
eth_diff = abs(latest["eth_price_chainlink"] - latest["eth_price_pyth"])
eth_diff_percentage = eth_diff / latest["eth_price_chainlink"] * 100

if eth_diff_percentage > 1:
    print(f"⚠️ 警告！ETH价格偏差异常！")
    print(f"Chainlink: ${latest['eth_price_chainlink']:.2f}")
    print(f"Pyth:      ${latest['eth_price_pyth']:.2f}")
    print(f"偏差：{eth_diff_percentage:.2f}%（超过1%阈值）")
else:
    print(f"✅ ETH价格偏差正常：{eth_diff_percentage:.2f}%")

# TVL偏差对比（Chainlink vs Defillama）
tvl_diff_chainlink = abs(latest["total_tvl_chainlink"] - latest["Defillama_tvl"])
tvl_diff_chainlink_percentage = tvl_diff_chainlink / latest["Defillama_tvl"] * 100

if tvl_diff_chainlink_percentage > 3:
    print(f"⚠️ 警告！Chainlink TVL与Defillama偏差异常！")
    print(f"Chainlink TVL: ${latest['total_tvl_chainlink']:,.2f}")
    print(f"Defillama TVL: ${latest['Defillama_tvl']:,.2f}")
    print(f"偏差：{tvl_diff_chainlink_percentage:.2f}%（超过3%阈值）")
else:
    print(f"✅ Chainlink TVL偏差正常：{tvl_diff_chainlink_percentage:.2f}%")

# TVL偏差对比（Pyth vs Defillama）
tvl_diff_pyth = abs(latest["total_tvl_pyth"] - latest["Defillama_tvl"])
tvl_diff_pyth_percentage = tvl_diff_pyth / latest["Defillama_tvl"] * 100

if tvl_diff_pyth_percentage > 3:
    print(f"⚠️ 警告！Pyth TVL与Defillama偏差异常！")
    print(f"Pyth TVL:      ${latest['total_tvl_pyth']:,.2f}")
    print(f"Defillama TVL: ${latest['Defillama_tvl']:,.2f}")
    print(f"偏差：{tvl_diff_pyth_percentage:.2f}%（超过3%阈值）")
else:
    print(f"✅ Pyth TVL偏差正常：{tvl_diff_pyth_percentage:.2f}%")
...
for log in logs[-7:]:  # 取最近7条rebase记录
    data = log["data"]
    ...
    apr_single = (post_share_price - pre_share_price) / pre_share_price * (365 * 24 * 3600 / timeElapsed)
    apr_list.append(apr_single)

apr_onchain_avg = sum(apr_list) / len(apr_list)
apy_onchain_avg = ((1 + apr_onchain_avg / 365) ** 365 - 1) * 100

print(f"链上7天平均APR：{apr_onchain_avg*100:.4f}%")
print(f"链上7天平均APY：{apy_onchain_avg:.4f}%")

# APR两源偏差预警
print("====== APR数据源偏差检测 ======")

apr_diff = abs(lido_apr - apr_onchain_avg * 100)
apr_diff_percentage = apr_diff / lido_apr * 100

if apr_diff_percentage > 5:
    print(f"⚠️ 警告！APR两源偏差异常！")
    print(f"Lido官方API APR：{lido_apr:.4f}%")
    print(f"链上7天平均APR：{apr_onchain_avg*100:.4f}%")
    print(f"偏差：{apr_diff_percentage:.2f}%（超过5%阈值）")
else:
    print(f"✅ APR两源偏差正常：{apr_diff_percentage:.2f}%")
    print(f"Lido官方API APR：{lido_apr:.4f}%")
    print(f"链上7天平均APR：{apr_onchain_avg*100:.4f}%")
...
today_real_8am=min(history1,key=lambda r:abs((datetime.strptime(r["timestamp"],"%Y-%m-%d %H:%M:%S")-target_today_8am).total_seconds()))#因为在实际过程中，可能实际记录不可能每次都正好是在八点整这个时间点记录数据，所以这里面我们使用了min函数，后面表示在history1这个列表里寻找离理想时间点八点整最近的这条记录，用today_real_8am命名，方便后面日环比分析引用。后面是规定的计算方式，用时间差计算为标准，用abs函数取绝对值，因为时间差可能是正数也可能是负数，我们只关心差值，不关心方向。
...
#chainlink数据的变化分析
eth_change_chainlink_real_8am=today_real_8am["eth_price_chainlink"]-yesterday_real_8am["eth_price_chainlink"]#用eth_change_chainlink_real_8am定义今天和昨天早上8点的eth价格差。
eth_change_chainlink_percentage_real_8am=eth_change_chainlink_real_8am/yesterday_real_8am["eth_price_chainlink"]*100#用eth_change_chainlink_percentage_real_8am定义今天和昨天早上8点的eth价格差占昨天eth价格变化的百分比。

tvl_change_chainlink_real_8am=today_real_8am["total_tvl_chainlink"]-yesterday_real_8am["total_tvl_chainlink"]#用tvl_change_chainlink_real_8am定义今天和昨天早上8点的总锁仓量差。
tvl_change_chainlink_percentage_real_8am=tvl_change_chainlink_real_8am/yesterday_real_8am["total_tvl_chainlink"]*100#用tvl_change_chainlink_percentage_real_8am定义今天和昨天早上8点的总锁仓量差占昨天总锁仓量变化的百分比。

#pyth数据的变化分析
eth_change_pyth_real_8am=today_real_8am["eth_price_pyth"]-yesterday_real_8am["eth_price_pyth"]#用eth_change_pyth_real_8am定义今天和昨天早上8点的eth价格差。
eth_change_pyth_percentage_real_8am=eth_change_pyth_real_8am/yesterday_real_8am["eth_price_pyth"]*100#用eth_change_pyth_percentage_real_8am定义今天和昨天早上8点的eth价格差占昨天eth价格变化的百分比。

tvl_change_pyth_real_8am=today_real_8am["total_tvl_pyth"]-yesterday_real_8am["total_tvl_pyth"]#用tvl_change_pyth_real_8am定义今天和昨天早上8点的总锁仓量差。
tvl_change_pyth_percentage_real_8am=tvl_change_pyth_real_8am/yesterday_real_8am["total_tvl_pyth"]*100#用tvl_change_pyth_percentage_real_8am定义今天和昨天早上8点的总锁仓量差占昨天总锁仓量变化的百分比。

#defillama数据的变化分析
defillama_change_tvl_real_8am=today_real_8am["Defillama_tvl"]-yesterday_real_8am["Defillama_tvl"]#用defillama_change_tvl_real_8am定义今天和昨天早上8点的总锁仓量差。
defillama_change_tvl_percentage_real_8am=defillama_change_tvl_real_8am/yesterday_real_8am["Defillama_tvl"]*100#用defillama_change_tvl_percentage_real_8am定义今天和昨天早上8点的总锁仓量差占昨天总锁仓量变化的百分比。
...
if hour == 8:
    ...
    # ETH价格单日涨跌预警（用chainlink为准）
    eth_change_abs = abs(eth_change_chainlink_percentage_real_8am)
    if eth_change_abs > 5:
        print(f"⚠️ 警告！ETH价格单日异常波动！")
        print(f"昨天：${yesterday_real_8am['eth_price_chainlink']:.2f} → 今天：${today_real_8am['eth_price_chainlink']:.2f}")
        print(f"涨跌幅：{eth_change_chainlink_percentage_real_8am:.2f}%（超过5%阈值）")
    else:
        print(f"✅ ETH价格波动正常：{eth_change_chainlink_percentage_real_8am:.2f}%")

    # TVL单日变化预警（用chainlink为准）
    tvl_change_abs = abs(tvl_change_chainlink_percentage_real_8am)
    if tvl_change_abs > 3:
        direction = "流入" if tvl_change_chainlink_real_8am > 0 else "流出"
        print(f"⚠️ 警告！TVL单日异常变化，资金大规模{direction}！")
        print(f"昨天：${yesterday_real_8am['total_tvl_chainlink']:,.2f} → 今天：${today_real_8am['total_tvl_chainlink']:,.2f}")
        print(f"变化幅度：{tvl_change_chainlink_percentage_real_8am:.2f}%（超过3%阈值）")
    else:
        print(f"✅ TVL变化正常：{tvl_change_chainlink_percentage_real_8am:.2f}%")

    # APR单日变化预警（需要昨天的APR，从daily_history里取）
    if len(daily_history) >= 2:
        yesterday_apr = daily_history[-2].get("apr_onchain", None)
        if yesterday_apr:
            apr_daily_change = abs(apr_onchain_avg * 100 - yesterday_apr) / yesterday_apr * 100
            if apr_daily_change > 20:
                print(f"⚠️ 警告！APR单日异常变化！")
                print(f"昨天APR：{yesterday_apr:.4f}% → 今天APR：{apr_onchain_avg*100:.4f}%")
                print(f"变化幅度：{apr_daily_change:.2f}%（超过20%阈值）")
            else:
                print(f"✅ APR变化正常：{apr_daily_change:.2f}%")

    # ETH价格和TVL背离预警
    eth_up = eth_change_chainlink_real_8am > 0
    tvl_up = tvl_change_chainlink_real_8am > 0
    if eth_up != tvl_up:  # 方向不一致就是背离
        if eth_up and not tvl_up:
            print(f"⚠️ 警告！ETH价格上涨但TVL下跌，用户可能在主动赎回stETH！")
        else:
            print(f"⚠️ 警告！ETH价格下跌但TVL上涨，注意核实数据是否异常！")
        print(f"ETH价格变化：{eth_change_chainlink_percentage_real_8am:.2f}%")
        print(f"TVL变化：{tvl_change_chainlink_percentage_real_8am:.2f}%")
    else:
        print(f"✅ ETH价格与TVL方向一致，无背离")

    print("==========================")

    # 每日推特情绪分析（每天早8点执行一次）
    print("====== X推文情绪分析 ======")
    sentiment_result = analyze_lido_sentiment()#调用上面定义好的情绪分析函数，拿到gpt对推文的分析结果，用sentiment_result命名，方便后面引用。
    print(sentiment_result)#打印出来gpt的情绪分析结果
    print("==============================")
...
9. 给出目前市场情绪的判断
10. 结合以下X平台推文情绪分析，判断链上数据与市场情绪是否一致，给出综合结论：
{sentiment_result}
"""}
        ]
...
# 以下是把通过gpt分析的日环比变化数据存到对应json文件的代码。
    ...
    daily_history.append({#把本次运行形成的record字典打包好的文本文件写入到daily_history这个列表里面。如果已经有数据就把本次数据写进翻译过后的列表的后面，如果没有数据，就直接写进空列表当第一个数据。
        ...
        "apr_onchain": apr_onchain_avg * 100,
        "tvl_chainlink": today_real_8am["total_tvl_chainlink"],
        "eth_change": eth_change_chainlink_real_8am,
        "tvl_change": tvl_change_chainlink_real_8am,
        "sentiment": sentiment_result#把今天的推特情绪分析结果也存进去，方便后面历史回顾和周报引用。
    })
    ...
    print(f"日报分析已保存到{daily_result_filename}")#提示我们日环比分析已经成功保存到对应位置了。
...
#====== 每周预警检测 ======
    ...
    tvl_list = [d["tvl_chainlink"] for d in week_daily if "tvl_chainlink" in d]
    apr_list = [d["apr_onchain"] for d in week_daily if "apr_onchain" in d]

    # TVL连续下跌预警
    tvl_drop_count = 0
    for i in range(1, len(tvl_list)):
        if tvl_list[i] < tvl_list[i-1]:
            tvl_drop_count += 1
        else:
            tvl_drop_count = 0

    if tvl_drop_count >= 3:
        print(f"⚠️ 警告！TVL已连续{tvl_drop_count}天下跌，资金持续外流！")
        print(f"本周TVL变化：${tvl_list[0]:,.2f} → ${tvl_list[-1]:,.2f}")
    else:
        print(f"✅ TVL无连续下跌异常，最长连跌{tvl_drop_count}天")

    # APR连续下降超过15%预警
    if len(apr_list) >= 2:
        apr_week_change = (apr_list[-1] - apr_list[0]) / apr_list[0] * 100
        if apr_week_change < -15:
            print(f"⚠️ 警告！本周APR持续下降！")
            print(f"周初APR：{apr_list[0]:.4f}% → 周末APR：{apr_list[-1]:.4f}%")
            print(f"下降幅度：{apr_week_change:.2f}%（超过15%阈值）")
        else:
            print(f"✅ APR周变化正常：{apr_week_change:.2f}%")

    # ETH和TVL背离天数预警
    divergence_count = 0
    for d in week_daily:
        if "eth_change" in d and "tvl_change" in d:
            eth_up = d["eth_change"] > 0
            tvl_up = d["tvl_change"] > 0
            if eth_up != tvl_up:
                divergence_count += 1

    if divergence_count >= 3:
        print(f"⚠️ 警告！本周ETH价格与TVL背离出现{divergence_count}次，用户行为异常！")
    else:
        print(f"✅ ETH与TVL背离次数正常：{divergence_count}次")
...
"""
    ...
    for 名字,内容 in prompts.items():#prompts.items()是把上面我们定义的prompts字典拆开，拆开成为引导词名称（key）对应名字变量，引导词内容（value）对应内容变量，这里学了新知识，in后面是两个变量for后面也可以接两个变量，分别对应key和value。
        ...
        gpt_response = client.chat.completions.create(#按照路径一步一步调用gpt5.4的功能，client是建立对话，chat是选择和他对话功能，completions是对话续写功能，create（）就是带入下面的具体数据，创建一次对话请求，就是请求做一下这个事情，整个过程状态用gpt_response命名，方便后面引用。
            ...
            messages=[
                ...
                "content": "你是一个专业的DeFi数据分析师，擅长分析Lido协议的TVL和ETH价格走势。"},
                {"role": "user",
                "content": 内容}#这里内容是可以变的变量，区别上面我们定死了引导内容，这里是根据不同的引导词名称，调用不同的引导内容。加了prompt前缀是为了和上面messages字典里的"content"键名区分，避免阅读时混淆。
            ]
        )
