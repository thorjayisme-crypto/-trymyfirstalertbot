...
def send_telegram(text):
    ...
    try:
        ...
        if result.get("ok"):
            ...
            print(f"⚠️ Telegram API返回错误：{result}")
...
def main():
    ...
    if len(history1)<2:
        ...
        hourly_alert_text = "⚠️ 历史数据不足，跳过小时预警分析"
    else:
        previous=history1[-2]
        latest=history1[-1]
        ...
        if tvl_hourly_change_percentage<-0.5:
            hourly_alert_text = f"🔴 TVL预警：过去1小时减少了{abs(tvl_hourly_change_percentage):.4f}%，大户都在跑路了，快跑！"
            print(f"链上总锁仓量在过去1小时内减少了{abs(tvl_hourly_change_percentage):.4f}%，🔴大户都在跑路了，快跑！")
        elif tvl_hourly_change_percentage<-0.1:
            hourly_alert_text = f"🟡 TVL预警：过去1小时减少了{abs(tvl_hourly_change_percentage):.4f}%，可能存在风险，需要注意了。"
            print(f"链上总锁仓量在过去1小时内减少了{abs(tvl_hourly_change_percentage):.4f}%，🟡可能存在风险，需要注意了。")
        else:
            hourly_alert_text = f"✅ TVL正常：过去1小时变化 {tvl_hourly_change_percentage:.4f}%，该吃吃该喝喝。"
            print(f"链上总锁仓量在过去1小时内变化不大，✅数据正常，该吃吃该喝喝。")

    send_telegram(f"""📊 Lido小时播报
时间：{now}

ETH价格（Chainlink）：${eth_price_chainlink:.2f}
总TVL（Chainlink）：${total_tvl_CHAINLINK:,.2f}
DefiLlama TVL：${Defillama_tvl:,.2f}
