...
def call_gpt(messages, model="gpt-5.5"):#我们定义一个叫call_gpt的函数，这个函数定义以后，后面需要调用gpt分析的地方就不用重复写一大堆api参数，直接引用call_gpt这个短代码就可以了。
    api_configs = [#设置了三个api接口用来备用，有失效了用备用继续完成任务，程序不会崩溃。我们把三个api接口写成一个列表，然后装进字典api_configs里面，方便后面引用。
        {"api_key": "sk-4e006feb80254c3bb7023aeb15e7dca5", "base_url": "https://right.codes/codex/v1"},
        {"api_key": "sk-ad5a18062e37625541db9f22d35ace52ce4c64dd0ed6fcb58284fa39a7c88b9d", "base_url": "https://ai.duomi.uk"},
        {"api_key": "sk-e8d5a613c64c0247c1c78834cb8a441e69c426f8fe2114b6e5a15b9d52e3d16d", "base_url": "https://api.aigocode.com"},
    ]
...
if w3 is None:#如果w3变量还是空集，就是说没有连接成功，就执行以下步骤。
    print("❌ 所有主网节点连接失败，程序停止")#表示所有api接口都连接失败了，打印出失败的原因。
    exit()#表示程序停止，因为主网都连不上就没必要运行整个程序了。   
...
try:#里面隐含逻辑，如果try里面程序运行成功，这个none空集就被替换成成功后获取的这个数据。
    ...
    print(f"⚠️ 获取总供应量失败：{e}，尝试下一个...")#表示try里面运行失败了，给我们报错，这里还是total_supply_eth=None
    exit()#总供应量都没拿到，程序还运行个鸡毛，直接停掉洗洗睡吧。
...
if eth_price_chainlink is None and eth_price_pyth is None:#这下子两个预言机都没拿到价格
    print("❌ 两个预言机都挂了，程序停止")#提醒我们失败了
    exit()#价格拿不到玩鸡毛，直接关程序吧
...
#以下是获取金库当日八点apr的代码
# 调用Lido官方API拿到APR
hour = datetime.now().hour#获取当前具体小时
...
"""}
        ...
        daily_result_filename = "C:/Users/86554/Desktop/CODING/lido_daily_analysis.json"
