...
from datetime import datetime#把datetime工具从datetime工具箱里拿出来，用来获取实时时间。
...
#以下是记录生成结果的代码
now=datetime.now().strftime("%Y-%m-%d %H:%M:%S")#这个相当于时间戳代码，把前面datetime工具拿到的实时时间从数据转化成标准格式的时间文本再方便记录。用now命名这个变量来方便后面引用。
record={
    "timestamp": now,
